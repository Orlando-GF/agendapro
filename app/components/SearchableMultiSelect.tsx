...
export function SearchableMultiSelect({ label, placeholder = 'BUSCAR...', items, value, onChange, erro }: Props) {
  const [aberto, setAberto] = useState(false)
  const [busca, setBusca] = useState('')
  const ref = useRef<HTMLDivElement>(null)

  const selecionados = items.filter(i => value.includes(i.id))
  ...
  const toggle = (id: string) => {
    const novo = value.includes(id) ? value.filter(v => v !== id) : [...value, id]
    onChange(novo)
  }
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        className={`w-full text-left rounded-lg border px-3 py-2 focus:outline-none focus:ring-2 bg-white min-h-[40px] ${
        ...
        {selecionados.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1">
            {selecionados.map(s => (
              <span
                key={s.id}
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-xs font-medium border border-blue-200"
              >
                {s.label}
                <span
                  role="button"
                  ...
                  className="text-blue-400 hover:text-blue-700 font-bold cursor-pointer select-none"
                >
                  ×
                </span>
              </span>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-blue-50 transition-colors flex items-center gap-2 ${
                    checked ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'
                  }`}
                >
